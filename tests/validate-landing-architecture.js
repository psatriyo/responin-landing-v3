...
assert(/<body class="v3-home"/.test(index), 'Homepage should use the V3 homepage shell');
assert(/class="container v3-hero-grid"/.test(index), 'Homepage must use the V2-inspired two-column hero grid');
assert(/id="demo"/.test(index) && /v3-ops-card/.test(index), 'Homepage must include the short operations demo card');
assert(/id="workflows"/.test(index), 'Homepage must include workflow use cases');
assert(/id="how"/.test(index), 'Homepage must include the operating loop');
assert(/id="safety"/.test(index), 'Homepage must include safety/human-control messaging');
...
assert(/\.v3-hero-grid/.test(css), 'CSS should include V3 hero-grid styles');
assert(/\.v3-workflow-grid/.test(css), 'CSS should include V3 workflow grid styles');
assert(/\.v3-safe-list/.test(css), 'CSS should include V3 safety list styles');
